"""
Independent cross-validation of symvb benzene Hubbard ground-state energies
...
In the s = 0 (orthogonal-AO) limit symvb returns the exact 400×400 symbolic
...
symvb works in a Slater-determinant VB basis with cofactor expansions;
...
def symvb_benzene_hubbard(t, U, _cache={}):
    """
    Ground-state energy from the cached symvb symbolic matrices.
...
def main():
    print(f"{'U/t':>6} {'E_symvb/|t|':>15} {'E_PySCF/|t|':>15} "
          f"{'|Δ| (a.u.)':>13} {'rel. err.':>12}")
    print('-' * 68)
    t = 1.0
    for U_over_t in [0.0, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0]:
        U = U_over_t * t
        E_symvb = symvb_benzene_hubbard(-t, U)  # h = -t convention
        E_pyscf = pyscf_benzene_hubbard(t, U)
        diff = abs(E_symvb - E_pyscf)
        rel = diff / max(abs(E_pyscf), 1e-12)
        print(f"{U_over_t:6.2f} {E_symvb:15.10f} {E_pyscf:15.10f} "
